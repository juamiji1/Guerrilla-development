...
foreach var of global migr{
	...
	*Table
	...
	outreg2 using "${tables}/rdd_migr_all.tex", tex(frag) keep(within_control) nor2 addstat("Bandwidth (Km)", ${h}, "Dependent mean", ${mean_y}) nonote nocons `r'
	local r append

}

preserve 
 insheet using "${tables}/rdd_migr_all.txt", nonames clear
insobs 3, before(1)
drop in 5
...
dataout, tex save("${tables}/rdd_migr_all_draft") replace nohead midborder(4)
restore 
