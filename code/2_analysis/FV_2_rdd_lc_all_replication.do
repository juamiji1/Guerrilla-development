...
clear all 
gl lcdata "${localpath}\Replication Package/Data (noGIS)/"

* Importing additional variables:

* State Administration
 import delimited "${localpath}/submissions/QJE/New Results/CabecerasCount.csv", clear 
 gen newstring  = string(seg_id,"%08.0f")
 drop seg_id
 rename newstring segm_id 
 rename numpoints cabecera
 save "${lcdata}/localcontinuity/Output Data/Cabeceras.dta", replace

* BECs in 1974:
import delimited "${lcdata}/localcontinuity/Output Data/BECs1974.csv", clear
...
save "${lcdata}/localcontinuity/Output Data/BECs1974.dta", replace

* Incarcerations in the Pre-period:
use "${lcdata}/incarcerations/entries_1980_1990_with_segment.dta", clear
 ...
 save "${lcdata}/localcontinuity/Output Data/Incarceration_Entries.dta", replace

* Presence of Military Bases
 import delimited "${localpath}/submissions/QJE/New Results/BasesMilitares.csv", clear 
 gen newstring  = string(seg_id,"%08.0f")
 drop seg_id
 rename newstring segm_id 
 rename numpoints basesmilitares
  save "${lcdata}/localcontinuity/Output Data/MilitaryBases.dta", replace


* Distance to Military Bases
import delimited "${localpath}/submissions/QJE/New Results/DisttoMilBase.csv", clear 
 gen newstring  = string(seg_id,"%08.0f")
 drop seg_id
 rename newstring segm_id 
 keep segm_id hubdist
 rename hubdist d_basesmilitares
   save "${lcdata}/localcontinuity/Output Data/DistancetoMilitaryBases.dta", replace
...
* State Administration:
 merge 1:1 segm_id using "${lcdata}/localcontinuity/Output Data/Cabeceras", nogen

* Incarcerations:
 merge 1:1 segm_id using "${lcdata}/localcontinuity/Output Data/Incarceration_Entries", nogen
 
 * Military Bases:
 merge 1:1 segm_id using "${lcdata}/localcontinuity/Output Data/MilitaryBases", nogen

 * Distance to Military Bases:
 merge 1:1 segm_id using "${lcdata}/localcontinuity/Output Data/DistancetoMilitaryBases", nogen

 * Hydrometrics and Land use 
 clonevar seg_id=segm_id
 merge 1:1 seg_id using "${lcdata}/localcontinuity/Output Data/shape_dhidro_landuse", nogen keepusing(desthidro landuse2)
...
merge m:1 d_name m_name using "${lcdata}/localcontinuity/Output Data/BECs1974.dta", gen(base_merge)
