...
*-------------------------------------------------------------------------------
*Global of outcomes
gl nl1 "arcsine_nl13 z_wi92 mean_educ_years92"
...
foreach var of global nl1{
	...
	*outreg2 using "${tables}/rdd_main_all_p1.tex", tex(frag) keep(within_control) addstat("Bandwidth (Km)", ${h}, "Dependent mean", ${mean_y}) label nonote nor2 nocons `r'
	local r append
}

/*
preserve 
  insheet using "${tables}/rdd_main_all_p1.txt", nonames clear
 drop v2
...
dataout, tex save("${tables}/rdd_main_all_p1") replace nohead midborder(2)
