...
gen sample2=sample1 // 150m donut hole
replace sample2=0 if abs(z_run_cntrl_v2)<=0.15

gen sample3=sample1 // 300m donut hole
replace sample3=0 if abs(z_run_cntrl_v2)<=0.3

gen sample4=sample1 // 450m donut hole
replace sample4=0 if abs(z_run_cntrl_v2)<=0.45

gen sample5=sample1 // 600m donut hole
replace sample5=0 if abs(z_run_cntrl_v2)<=0.6
...
foreach var of global nl1{
	...
	*Table
	...
	outreg2 using "${tables}/rdd_main_`var'_5kmBDW_DH.tex", tex(frag) keep(within_control) addstat("Bandwidth (Km)", ${h}, "Dependent mean", ${mean_y}, "Donut Hole (Km)",${dh}) label nonote nor2 nocons `r'
	local r append
	gl dh=${dh}+0.150
	}
...
preserve 
 insheet using "${tables}/rdd_main_`var'_5kmBDW_DH.txt", nonames clear
save "${tables}/rdd_main_`var'_5kmBDW_DH" , replace 
restore 
}

preserve 
 insheet using "${tables}/rdd_main_arcsine_nl13_5kmBDW_DH.txt", nonames clear
 append using "${tables}/rdd_main_z_wi_5kmBDW_DH"
 append using "${tables}/rdd_main_mean_educ_years_5kmBDW_DH"
...
dataout, tex save("${tables}/rdd_main_5kmBDW_DH") replace nohead  midborder(1)
...
erase "${tables}/rdd_main_`var'_5kmBDW_DH.dta" 
