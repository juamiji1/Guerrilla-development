...
cap erase "${tables}\rdd_main_all_rural.tex"
cap erase "${tables}\rdd_main_all_rural.txt"

foreach var of global nl1{
	
	*Table
	reghdfe `var' ${controls} [aw=tweights] ${if} & AREA_I=="R" , vce(r) a(i.${breakfe}) resid
	summ `var' if e(sample)==1 & within_control==0, d
	gl mean_y=round(r(mean), .01)
	
	outreg2 using "${tables}\rdd_main_all_rural.tex", tex(frag) keep(within_control) addtext("Kernel", "Triangular") addstat("Bandwidth (Km)", ${h},"Polynomial", 1, "Dependent mean", ${mean_y}) label nonote nocons append 
