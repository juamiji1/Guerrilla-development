...
*Collapsing 
collapse (mean) z_index* rdsample index* (first) within_control, by(segm_id)
*collapse (mean) index* rdsample, by(segm_id)
  ...
  foreach y in index_pp index_ep index_ap index_trst {
  	...
  	gen z_`y'_v3=((`y'-r(mean))/r(sd)) if rdsample==1
 }
summ z_index_pp_v3 z_index_ep_v3 z_index_ap_v3 z_index_trst_v3 if rdsample==1 & within_control==0
...
foreach var of global trst{
	...
	*Table
	...
	summ `var' if rdsample==1 & within_control==0
...
replace v7 = "" in 2
replace v7 = "Distrust of Members of" in 1
replace v7 = "the Community (Share)" in 2

dataout, tex save("${tables}/rdd_trustsum_all") replace nohead  midborder(3)
