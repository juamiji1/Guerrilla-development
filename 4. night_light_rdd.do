...
gen z_run_cntrl_v4= dist_control 
replace z_run_cntrl_v4= -1*dist_control if within_control_v3==0 

gen z_run_cntrl_v5= z_run_cntrl 
replace z_run_cntrl_v5= z_run_cntrl-0.01 if z_run_cntrl==0 
...
rdrobust nl13_density z_run_cntrl if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular) 
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection") addstat("Polynomial", 1) nonote replace  

rdrobust nl13_density z_run_cntrl_v2 if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular)
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid") addstat("Polynomial", 1) nonote append  
...
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid", "Note", "cut-off: -0.2") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density z_run_cntrl_v3 if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular)
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection", "Note", "cut-off: deleted obs") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density_v2 z_run_cntrl if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular)
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection", "Note", "p1-p99") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density_v2 z_run_cntrl_v2 if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular) 
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid", "Note", "p1-p99") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density_v3 z_run_cntrl if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular)
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection", "Note", "p5-p95") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density_v3 z_run_cntrl_v2 if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular) 
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid", "Note", "p5-p95") addstat("Polynomial", 1) nonote append  

rdrobust ln_nl13 z_run_cntrl if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular)
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("ln(Night light)") addtext("Method", "Intersection") addstat("Polynomial", 1) nonote append  

rdrobust ln_nl13 z_run_cntrl_v2 if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular) 
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("ln(Night light)") addtext("Method", "Centroid") addstat("Polynomial", 1) nonote append  

rdrobust ln_nl13_plus z_run_cntrl if within_expansion==0 & (within_control==1 | within_disputed==1), all kernel(triangular)
outreg2 using "${tables}\rdd_cntrl_robustness.tex", tex(frag) ctitle("ln(Night light+0.01)") addtext("Method", "Intersection") addstat("Polynomial", 1) nonote append  
...
rdrobust nl13_density z_run_dsptd if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection") addstat("Polynomial", 1) nonote replace  

rdrobust nl13_density z_run_dsptd_v2 if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density z_run_dsptd_v2 if within_expansion==0 & within_control==0, all kernel(triangular) c(-0.2)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid", "Note", "cut-off: -0.2") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density z_run_dsptd_v3 if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection", "Note", "cut-off: deleted obs") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density_v2 z_run_dsptd if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection", "Note", "p1-p99") addstat("Polynomial", 1) nonote append 

rdrobust nl13_density_v2 z_run_dsptd_v2 if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid", "Note", "p1-p99") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density_v3 z_run_dsptd if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Intersection", "Note", "p5-p95") addstat("Polynomial", 1) nonote append  

rdrobust nl13_density_v3 z_run_dsptd_v2 if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("Night light") addtext("Method", "Centroid", "Note", "p5-p95") addstat("Polynomial", 1) nonote append  

rdrobust ln_nl13 z_run_dsptd if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("ln(Night light)") addtext("Method", "Intersection") addstat("Polynomial", 1) nonote append  

rdrobust ln_nl13 z_run_dsptd_v2 if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("ln(Night light)") addtext("Method", "Centroid") addstat("Polynomial", 1) nonote append  

rdrobust ln_nl13_plus z_run_dsptd if within_expansion==0 & within_control==0, all kernel(triangular)
outreg2 using "${tables}\rdd_dsptd_robustness.tex", tex(frag) ctitle("ln(Night light+0.01)") addtext("Method", "Intersection") addstat("Polynomial", 1) nonote append  
