		...
		foreach m of global bwmethod {
			foreach w of global weights {
				...
				*Estimation 
				...
				outreg2 using "${tables}/rdd_`var'_mainrobust_all_p`p'.tex", tex(frag) keep(within_control) addtext("Bw type","`m'", "Kernel", "`w'") addstat("Bandwidth (Km)", ${h},"Polynomial", `p', "Dependent mean", ${mean_y}) label nonote nocons `r'
