...
gl controls "within_control x_coord y_coord"
...
foreach var of global nl1{
	...
	*Table
	...
	outreg2 using "${tables}/latlon.tex", tex(frag) keep(within_control) addstat("Bandwidth (Km)", ${h}, "Dependent mean", ${mean_y}) label nonote nor2 nocons `r'
	local r append
...
preserve 
  insheet using "${tables}/latlon.txt", nonames clear
 drop v2
...
dataout, tex save("${tables}/latlon") replace nohead midborder(3)
